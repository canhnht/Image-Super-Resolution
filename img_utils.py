...
def transform_images(directory, output_directory, scaling_factor=2, max_nb_images=-1, true_upscale=False):
    ...
    for file in os.listdir(directory):
        ...
        image_subsample_iterator = subimage_generator(
            img, stride, hr_patch_size, nb_hr_images)
        ...
        for i in range(nb_hr_images):
            ...
            if not true_upscale:
                ...
                op = imresize(op, (hr_patch_size, hr_patch_size),
                              interp='bicubic')

            # Save Y
            imsave(output_directory + "/X/" + "%d_%d.png" % (index, i+1), op)

        print("Finished image %d in time %0.2f seconds. (%s)" %
              (index, time.time() - t1, file))
        index += 1
...
def transform_images_temp(directory, output_directory, scaling_factor=2, max_nb_images=-1, true_upscale=False,
    ...
    for file in os.listdir(directory):
        ...
        for i in range(nb_hr_images):
            ip = hr_samples[i]
            # Save ground truth image X
            imsave(output_directory + "/y/" + "%d_%d.png" %
                   (index + id_advance, i + 1), ip)
            ...
            # Save Y
            imsave(output_directory + "/X/" + "%d_%d.png" %
                   (index + id_advance, id_advance + i + 1), op)

        print("Finished image %d in time %0.2f seconds. (%s)" %
              (index + id_advance, time.time() - t1, file))
        index += 1
...
def image_generator(directory, scale_factor=2, target_shape=None, channels=3, small_train_images=False, shuffle=True,
    ...
    if not target_shape:
        if small_train_images:
            if K.image_dim_ordering() == "th":
                image_shape = (channels, 16 * _image_scale_multiplier,
                               16 * _image_scale_multiplier)
                ...
                image_shape = (32 * _image_scale_multiplier,
                               32 * _image_scale_multiplier, channels)
                y_image_shape = (32 * scale_factor * _image_scale_multiplier,
                                 32 * scale_factor * _image_scale_multiplier, channels)
        else:
            if K.image_dim_ordering() == "th":
                image_shape = (channels, 32 * scale_factor * _image_scale_multiplier,
                               32 * scale_factor * _image_scale_multiplier)
                y_image_shape = image_shape
            else:
                image_shape = (32 * _image_scale_multiplier, 32 * _image_scale_multiplier,
                               channels)
    ...
    print("image_shapeeee", image_shape, y_image_shape)
    ...
    while 1:
        ...
        for i, j in enumerate(index_array):
            ...
            if small_train_images:
                img = imresize(img, (32 * _image_scale_multiplier,
...
if __name__ == "__main__":
    ...
    transform_images(validation_set5_path, validation_output_path, scaling_factor=scaling_factor, max_nb_images=-1,
                     true_upscale=true_upscale)
    transform_images(validation_set14_path, validation_output_path, scaling_factor=scaling_factor, max_nb_images=-1,
                     true_upscale=true_upscale)
    pass
